...
> **Эта ветка устарела.** Актуальная работа — в ветке
> [`build-optimization`](../../tree/build-optimization), она же ветка по
> умолчанию. Здесь лежит серия из 29 патчей и старая конфигурация сборки,
> которая пересобирала приложение дважды и не укладывалась в лимиты. Все
> автоматические запуски в этой ветке отключены.
